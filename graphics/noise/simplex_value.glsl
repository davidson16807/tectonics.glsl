 ...
 */
void get_3d_simplex_vertex_index_offsets( 
    const in vec3 P, 
    out vec3 offset1, 
    out vec3 offset2 
){
...
float get_3d_simplex_perlin_noise(
    ...
    vec3 O1; vec3 O2; get_3d_simplex_vertex_index_offsets(F, O1, O2);
    // simplex vertex indices
    vec3 I0 = floor(S*V); 
    vec3 I1 = I0 + O1;
    vec3 I2 = I0 + O2;
 ...
 * Efficient simplex indexing functions by Bill Licea-Kane, ATI. Thanks!
 */
void get_4d_simplex_vertex_index_offsets( 
    const in vec4 P, 
    out vec4 offset1, 
    out vec4 offset2, 
    out vec4 offset3 
...
float get_4d_simplex_perlin_noise(
    ...
    vec4 F  = fract(S*V);
    // simplex vertex index offsets
    vec4 O1; vec4 O2; vec4 O3; get_4d_simplex_vertex_index_offsets(F, O1, O2, O3);
    // simplex vertex indices
    vec4 I0 = floor(S*V); 
    vec4 I1 = I0 + O1;
    vec4 I2 = I0 + O2;
